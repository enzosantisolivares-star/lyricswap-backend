...
async def swap_lyrics(
    ...
    try:
        ...
        result = subprocess.run(["python","-m","demucs","--two-stems=vocals","-o",str(demucs_output),str(original_path)], capture_output=True, text=True, timeout=300)
        ...
        if not vocals_path or not no_vocals_path:
            raise HTTPException(status_code=500, detail="Demucs no genero archivos")
        ...
        prompt = f"Reescribe esta letra manteniendo el ritmo y metrica exacta.\nLETRA ORIGINAL:\n{original_lyrics}\nNUEVO TEMA:\n{theme}\nResponde SOLO la nueva letra."
        ...
        el_response = requests.post(f"https://api.elevenlabs.io/v1/text-to-speech/{voice_id}", headers={"xi-api-key":ELEVENLABS_API_KEY,"Content-Type":"application/json"}, json={"text":new_lyrics,"model_id":"eleven_multilingual_v2","voice_settings":{"stability":0.5,"similarity_boost":0.8}}, timeout=120)
        ...
        ffmpeg_result = subprocess.run(["ffmpeg","-y","-i",str(no_vocals_path),"-i",str(new_vocals_path),"-filter_complex","[0:a]volume=1.0[inst];[1:a]volume=1.2[voc];[inst][voc]amix=inputs=2:duration=longest:dropout_transition=2[out]","-map","[out]","-ar","44100","-ab","192k",str(output_path)], capture_output=True, text=True, timeout=120)
        if ffmpeg_result.returncode != 0:
            raise HTTPException(status_code=500, detail=f"FFmpeg error: {ffmpeg_result.stderr}")
        return FileResponse(path=str(output_path), media_type="audio/mpeg", filename="lyricswap_output.mp3", headers={"X-Original-Lyrics":original_lyrics[:500],"X-New-Lyrics":new_lyrics[:500]})
